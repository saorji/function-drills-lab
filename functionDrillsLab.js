...
//CODE HERE
function nameCheck(paraname){
  if(paraname === 'Steven'){
    return (`What is up ${paraname}`)
  }else if (paraname === 'Bryan'){
    return (`Hey ${paraname}`)
  }else {
    return (`Cool name, ${paraname}`)
  }
}
...
function printAllNames(arrnames){
  for (let i= 0; i<=namesArr.length-1; i++){
    console.log(`passing in the ${namesArr[i]} array`)
  }
